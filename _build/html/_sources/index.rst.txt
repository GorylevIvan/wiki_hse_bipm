...
Целями освоения данной дисциплины являются как закрепление теоретических знаний в области объектно-ориентированного программирования, так и получение практических навыков программирования на языке JAVA с использованием стандартных средств разработки.
...
- **Структура класса**:
  ...
  .. code-block:: none
     ...
     [модификатор] class [ИмяКласса] {
         [модификатор] [тип] [поле];
         [модификатор] [тип] [метод]([аргументы]) { ... }
     }
...
- **Методы**: length(), charAt(), substring(), indexOf(), equals()
- **Пример**:
...
- **Методы**: openStream(), getProtocol(), getHost(), getPort()
- **Пример**:
...
- **Методы**: getInputStream(), getOutputStream(), setDoOutput(true)
- **Пример (POST-запрос)**:
...
- **Методы**: setRequestMethod("GET/POST"), getResponseCode()
- **Пример**:
...
Лекция 10. Многопоточность (Threads & Concurrency)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Лекция 11. Лямбда-выражения и потоки (Lambdas & Streams)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Лекция 12. Аннотации (Annotations)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Программа должна принимать у пользователя два имени файла: входной и выходной. Далее открывается входной файл (если существует), подсчитывается кол-во различных символов английского алфавита A-Z, a-z и результаты подсчета по каждому символу записываются в выходной файл. Исключения обрабатывать, кодинг стайл соблюдать.
...
Есть многоэтажный жилой дом, в нем 2 (опционально 3) лифта. Рандомно с разных этажей поступают вызовы лифта. Лифт может останавливаться по попутным вызовам. Реализовать программу управления лифтами, минимизирующую время ожидания лифта после поступления вызова и пробег лифта. Наличие визуализации +1 балл.
...
5. Выберите верное утверждение: Если член класса объявлен с модификатором protected, доступ к нему возможен?
...
9. Выберите верное утверждение: Класс, унаследованный от абстрактного.
...
12. Дан код .Какое выражение в выделенной строке создаст экземпляр вложенного класса?
    
13. Дан класс для хранения комплексных чисел. Какой метод класса Object надо переопределить в классе Complex чтобы можно было выводить комплексные числа на экран методом System.out.println()?
...
18. Какой из интерфейсов предоставляет возможность хранить объекты в виде пары "ключ-значение"?

19. В каком случае ThreadPoolExecutor будет создавать новый поток при поступлении новой задачи?

20. Члены класса с какими модификаторами НЕ подлежат сериализации?  
